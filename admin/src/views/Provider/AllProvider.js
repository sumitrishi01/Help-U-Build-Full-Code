...
function AllProvider() {
    ...
    const updateIsBanned = async (id, currentStatus) => {
        ...
        try {
            ...
            const res = await axios.put(
                `https://try.helpubuild.co.in/api/v1/update-provider-isbanned/${id}`,
    ...
    const handleDelete = async (id) => {
        setLoading(true);
        try {
            await axios.delete(`https://try.helpubuild.co.in/api/v1/delete-provider/${id}`);
            fetchProviders();
    ...
    const handleAccountVerification = async (status, providerId = selectedProvider) => {
        ...
        try {
            const res = await axios.put(
                `https://try.helpubuild.co.in/api/v1/provider_verify/${providerId}`,
