...
function AllChatRoom() {
    ...
    const handleFetchChat = async (chatRoomId) => {
        try {
            const { data } = await axios.get(`https://try.helpubuild.co.in/api/v1/get-chat-by-room/${chatRoomId}`);
    ...
    const handleDeleteBanner = async (id) => {
        setLoading(true);
        try {
            await axios.delete(`https://try.helpubuild.co.in/api/v1/delete-chat-room/${id}`);
            handleFetchBanner();
