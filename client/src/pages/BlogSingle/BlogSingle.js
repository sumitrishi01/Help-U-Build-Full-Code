...
const BlogSingle = () => {
  ...
  const fetchBlogData = async () => {
    try {
      const { data } = await axios.get(`https://try.helpubuild.co.in/api/v1/get-single-blog/${id}`)
      setData(data.data)
    } catch (error) {
  ...
  const handleComment = async (e) => {
    ...
    try {
      const res = await axios.post('https://try.helpubuild.co.in/api/v1/create-blog-comment', newFormData)
