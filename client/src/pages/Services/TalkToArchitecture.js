...
function TalkToArchitect() {
  ...
  useEffect(() => {
    const handleFetchProviderAllService = async () => {
      try {
        setLoading(true)
        const all = await axios.get('https://try.helpubuild.co.in/api/v1/get-all-provider-service');
  ...
  const handleFetchProviderService = async (providerId) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `https://try.helpubuild.co.in/api/v1/get-service-by-provider/${providerId}/Residential`
      );
