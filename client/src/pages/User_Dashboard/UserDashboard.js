...
const UserDashboard = () => {
  ...
  const GetMyProfile = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.get(`https://try.helpubuild.co.in/api/v1/get-single-provider/${providerId}`);
      // console.log(data)
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post('https://try.helpubuild.co.in/api/v1/addPortfolio?type=Portfolio', formData, {
        headers: {
  ...
  const handleFetchCommission = async () => {
    try {
      const {data} = await axios.get('https://try.helpubuild.co.in/api/v1/get-all-commision')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("https://try.helpubuild.co.in/api/v1/create-withdraw-request", {
